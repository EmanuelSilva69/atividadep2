...
void segundo_maior_valor_vetor_inteiros(){
  ...
  for(ptr=vetor;ptr<vetor+3;ptr++){
    c=*ptr;
    for(ptr2=vetor+1;ptr2<vetor+4;ptr2++){
      if(*ptr2<c){
        c=ptr2;
      }
    }
  ...
  for (ptr = vetor+3; ptr < vetor + 4; ptr++) {
      printf("%d", *ptr);
  }
}
...
void verifica_palindromo(){
   int vetor[10];
    ...
    i=10;
    p=0;
    for (ptr = vetor; ptr < vetor + 10; ptr++) {
        scanf("%d", ptr);
    }
  ptr=vetor;
  ptr2=(vetor+9);
